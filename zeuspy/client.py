...
import zeuspy
from zeuspy import __version__, __license__
from zeuspy import enums
from zeuspy import raw
from zeuspy import utils
from zeuspy.crypto import aes
from zeuspy.errors import CDNFileHashMismatch
from zeuspy.errors import (
    SessionPasswordNeeded,
    VolumeLocNotFound, ChannelPrivate,
    AuthBytesInvalid, BadRequest
)
from zeuspy.handlers.handler import Handler
from zeuspy.methods import Methods
from zeuspy.session import Auth, Session
from zeuspy.storage import FileStorage, MemoryStorage
from zeuspy.types import User, TermsOfService
from zeuspy.utils import ainput
...
class Client(Methods):
    """zeuspy Client, the main means for interacting with Telegram.
    ...
    Parameters:
        ...
        in_memory (``bool``, *optional*):
            ...
            :meth:`~zeuspy.Client.export_session_string` before stopping the client to get a session string you can
        ...
        workdir (``str``, *optional*):
            ...
            The working directory is the location in the filesystem where zeuspy will store the session files.
    ...
    APP_VERSION = f"zeuspy {__version__}"
    ...
    async def authorize(self) -> User:
        ...
        print(f"Welcome to zeuspy (version {__version__})")
        print(f"zeuspy is free software and comes with ABSOLUTELY NO WARRANTY. Licensed\n"
    ...
    def set_parse_mode(self, parse_mode: Optional["enums.ParseMode"]):
        ...
        Parameters:
            parse_mode (:obj:`~zeuspy.enums.ParseMode`):
        ...
        if session_empty:
            if not self.api_id or not self.api_hash:
                raise AttributeError("The API key is required for new authorizations. "
                                     "More info: https://docs.zeuspy.org/start/auth")
    ...
    async def handle_download(self, packet):
        ...
        try:
            async for chunk in self.get_file(file_id, file_size, 0, 0, progress, progress_args):
                file.write(chunk)
        except zeuspy.StopTransmission:
            if not in_memory:
                file.close()
                os.remove(file.name)
