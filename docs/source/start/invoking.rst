...
At this point, we have successfully :doc:`installed zeuspy <../intro/install>` and :doc:`authorized <auth>` our
...
Making API calls with zeuspy is very simple. Here's a basic example we are going to examine step by step:
...
#.  Finally, we tell Python to schedule our ``main()`` async function by using zeuspy's :meth:`~zeuspy.Client.run`
    method.
...
the Client, asynchronously. It does so by automatically calling :meth:`~zeuspy.Client.start` and
:meth:`~zeuspy.Client.stop` in a more convenient way which also gracefully stops the client, even in case of
...
Alternatively to the :meth:`~zeuspy.Client.run` method, you can use Python's ``asyncio.run()`` to execute the main
