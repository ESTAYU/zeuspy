...
Once a :doc:`project is set up <setup>`, you will still have to follow a few steps before you can actually use zeuspy to make
...
zeuspy automatically manages this process, all you need to do is create an instance of the
:class:`~zeuspy.Client` class by passing to it a ``name`` of your choice (e.g.: "my_account") and call
the :meth:`~zeuspy.Client.run` method:
...
After successfully authorizing yourself, a new file called ``my_account.session`` will be created allowing zeuspy to
...
:doc:`configure a Telegram API key <../start/setup>` with zeuspy, even when using bots.
