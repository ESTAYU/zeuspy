...
zeuspy embeds a smart, lightweight yet powerful plugin system that is meant to further simplify the organization
of large projects and to provide a way for creating pluggable (modular) components that can be easily shared across
different zeuspy applications with minimal boilerplate code.
...
.. note::
    ...
    Example: *"zeuspy"* replies with *"zeuspy"* and *"margoryP"*
...
manually ``import``, manually :meth:`~zeuspy.Client.add_handler` and manually instantiate each
:class:`~zeuspy.handlers.MessageHandler` object because you can't use decorators for your functions.
So, what if you could? Smart Plugins solve this issue by taking care of handlers registration automatically.

Using Smart Plugins
-------------------

Setting up your zeuspy project to accommodate Smart Plugins is pretty straightforward:
...
The second thing is telling zeuspy where to look for your plugins: you can use the Client parameter "plugins";
the *root* value must match the name of your plugins root folder. Your zeuspy Client instance will **automatically**
...
:meth:`~zeuspy.Client.remove_handler` Client's method with your function's *handler* instance:
...
using :meth:`~zeuspy.Client.add_handler` instead. Example:
