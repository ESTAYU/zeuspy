...
zeuspy already provides lots of built-in :class:`~zeuspy.filters` to work with, but in case you can't find a
specific one for your needs or want to build a custom filter by yourself you can use
:meth:`filters.create() <zeuspy.filters.create>`.
...
:class:`~zeuspy.handlers.CallbackQueryHandler`. Note that callback queries updates are only received by bots as result
...
.. code-block:: python
    ...
    await app.send_message(
        ...
        "zeuspy custom filter test",
        reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton("Press me", "zeuspy")]]
        )
    )
...
For this basic filter we will be using only the first parameter of :meth:`~zeuspy.filters.create`.
...
In this example we are matching the query data to "zeuspy", which means that the filter will only allow callback
queries containing "zeuspy" as data:
...
.. code-block:: python
    ...
    async def zeuspy_data(_, query):
        query.answer("it works!")

Filters with Arguments
----------------------

A more flexible filter would be one that accepts "zeuspy" or any other string as argument at usage time.
A dynamic filter like this will make use of named arguments for the :meth:`~zeuspy.filters.create` method and the
...
.. code-block:: python
    ...
    @app.on_callback_query(dynamic_data_filter("zeuspy"))
    async def zeuspy_data(_, query):
        query.answer("it works!")
...
argument. This is a reference to the :obj:`~zeuspy.Client` instance that is running the filter and it is useful in
